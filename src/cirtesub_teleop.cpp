...
public:
  ...
  : Node("cirtesub_teleop")
  {
    ...
    zip_tie_auto_after_zip_wait_ms_ =
      std::max(0, static_cast<int>(get_parameter("zip_tie_auto_after_zip_wait_ms").as_int()));
...
private:
  ...
  {
    if (zip_tie_auto_state_ != ZipTieAutoState::Idle) {
      RCLCPP_WARN(get_logger(), "Zip tie automation already running.");
      return;
    }
    ...
    requestControllerStates(
      ...
      {
        const bool gripper_active =
          isControllerActive(controller_states, newton_gripper_controller_name_);
        const bool zip_active =
          isControllerActive(controller_states, servo_zip_controller_name_);

        if (!gripper_active || !zip_active) {
          RCLCPP_WARN(get_logger(), "Zip tie automation blocked: gripper or zip controller not active.");
          return;
        }
  ...
  void updateZipTieAutomation()
  {
    ...
    if (zip_tie_auto_state_ == ZipTieAutoState::InitialClosingGripper && now >= zip_tie_next_time_) {
      ...
      scheduleZipTieStep(zip_tie_auto_after_zip_wait_ms_);
      zip_tie_auto_state_ = ZipTieAutoState::WaitingAfterZip;
    } else if (zip_tie_auto_state_ == ZipTieAutoState::WaitingAfterZip && now >= zip_tie_next_time_) {
      ...
      zip_tie_auto_state_ = ZipTieAutoState::Idle;
      RCLCPP_INFO(get_logger(), "Zip tie automation complete.");
    }
  }
  ///
  ...
  //// auto
  enum class ZipTieAutoState { Idle, InitialClosingGripper, PushingZip, WaitingAfterZip, FinalClosingGripper, OpeningGripper };
  ...
  int zip_tie_auto_after_zip_wait_ms_{5000};  
